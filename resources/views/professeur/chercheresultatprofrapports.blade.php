...
<header class="header">
   <section class="flex">
      ...
      <form action="{{ route('chercherprofrapports') }}" method="post" class="search-form">
         @csrf
         <input type="text" name="search_box" required placeholder="chercher rapports" maxlength="100">
...
<table class="tablesearch">
    ...
    <tbody>
        @foreach($rapport as $ra)
        <tr>
            <td>{{ $ra->nom_professeur}}</td>
            <td><a href="{{ url('' . $ra->pdf_path) }}"><i class="fa-solid fa-file-pdf"></i> Rapport</a></td>
